---
title: "gpt_segmentation_analysis"
...
```{r}
data_path = 'PATH_TO/'
gpt_data = read_csv(paste0(data_path, 'gpt_segmentation_data.csv')) %>% 
  mutate(story_id = as.factor(story_id),
  			 temperature = as.factor(temperature))

gpt_series = read_csv(paste0(data_path,'gpt_time_word_series.csv')) %>% 
	  mutate(story_id = as.factor(story_id),
  			 temperature = as.factor(temperature))
...
```{r}
grouped_data = gpt_series %>% 
  group_by(story_id, temperature) %>% 
  group_split()

agreement_gpt = tibble()
...
for (df in grouped_data){
  ...
  agreement_gpt = rbind(agreement_gpt, temp)
}
```
...
agreement_model = agreement %>% 
  lmer(agreement_index ~ condition + (1 + condition | subject) + (1 | story_id), data = .) 
...
## Human-GPT Agreement Index

Human subjects compared to the GPT produced group event series.
...
for (df in grouped_data){
  ...
  agreement_gpt_human = visual_series %>% 
...
```{r}
agreement_model = agreement_gpt_human %>% 
